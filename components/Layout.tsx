        ...
        <div className="min-h-screen text-white relative font-sans selection:bg-primary selection:text-white">
                ...
                <div className="max-w-[1920px] mx-auto flex items-center justify-between">
                    ...
                    <Link to="/" className="relative group z-50">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 md:w-12 md:h-12 bg-white text-black rounded-xl flex items-center justify-center font-black text-xl md:text-2xl shadow-[0_0_30px_rgba(255,255,255,0.3)] group-hover:scale-110 transition-transform duration-500">
                                C
                            </div>
                    ...
                    <div className="flex items-center gap-4 z-50">
                                        ...
                                        <>
                                            {searchPreview.map(item => (
                                                <button
                                                    ...
                                                    className="w-full p-4 flex items-center gap-4 hover:bg-white/10 transition-colors text-left border-b border-white/5 last:border-0"
                                                >
                                                    <img src={item.poster} className="w-10 h-14 object-cover rounded-md bg-white/5" alt="" />
                                                    <div className="min-w-0">
                                                        <div className="text-sm font-bold truncate">{item.title}</div>
                                                        <div className="text-[9px] uppercase tracking-wider text-white/40">{item.year} • {item.category}</div>
                                                    </div>
                                                </button>
                                            ))}
                                            <button 
                                                onClick={(e) => handleSearch(e as any)}
                                                className="w-full p-3 text-[10px] font-bold uppercase tracking-widest text-center hover:bg-white/10 transition-colors text-primary-light"
                                            >
                                                View All Results
                                            </button>
                                        </>
                                    ) : (
                                        <div className="p-6 text-center text-xs font-bold uppercase tracking-widest text-white/30">
                                            No matches found
                                        </div>
                                    )}
                                </div>
                        ...
                        <Link 
                            to="/admin" 
                            className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-2xl glass bg-white/5 border border-transparent hover:border-white/20 hover:bg-white/10 transition-all ${location.pathname === '/admin' ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`}
                            title="Admin Panel"
                        >
                            <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <footer className="relative z-10 py-12 text-center text-white/20">
                <div className="flex items-center justify-center gap-2 mb-4">
                    ...
                    <div className="text-[10px] font-bold uppercase tracking-[0.3em]">Cinematic Odyssey</div>
                    <div className="w-1 h-1 rounded-full bg-white/20"></div>
                </div>
                <div className="text-[9px] font-bold uppercase tracking-widest">
                    &copy; {new Date().getFullYear()} Personal Cinema Database
                </div>
            </footer>
        </div>
    );
